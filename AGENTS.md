...
1. **Active design — five themes, know which one you're editing** (all use `assets/js/main.js`):
   ...
   - **Homepage toolkit** — a "Tools I Work With" logo section (added later, in `index.html` + `home.css`) using 15 logo `.webp` files at the repo **root** (e.g. `chatgpt.webp`, `n8n.webp`, `gemini logo.webp`). Don't move them; root pages reference them relative.
